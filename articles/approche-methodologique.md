...
Établir un portrait de la disponibilité des données génomiques pour les
espèces observées au Québec (liste BDQC, ~24 800 espèces), en
interrogeant BOLD Systems et NCBI, puis en croisant les résultats avec
les statuts de conservation (COSEPAC/LEMV).
...
[`targets`](https://books.ropensci.org/targets/) (`_targets.R`, à la
racine du dépôt) : chaque étape est une fonction exportée du package,
mise en cache par `targets` (une étape n’est ré-exécutée que si son code
ou ses entrées ont changé — utile puisque plusieurs étapes sont des
appels API lents et soumis au rate limiting).
...
par espèce) interroge les bases `genome` (génomes nucléaires) et
`nucleotide` (génomes mitochondriaux complets) pour chaque espèce de la
liste BDQC.
...
récupère les annotations de gènes (XML GenBank) pour un sous-échantillon
d’accessions par espèce.
[`assign_gene_group()`](https://taq-community.github.io/QCGenomLandscape/reference/assign_gene_group.md)
classe chaque gène en marqueur standardisé (COI, Cytb, ND1/2/4/5,
12S/16S rRNA, ITS, marqueurs photosynthétiques).
[`classify_taxon_group()`](https://taq-community.github.io/QCGenomLandscape/reference/classify_taxon_group.md)
affine les groupes taxonomiques BDQC (ex. mammifères marins
vs. terrestres, insectes vs. crustacés).

### 5. Statuts de conservation

[`load_risk_status()`](https://taq-community.github.io/QCGenomLandscape/reference/load_risk_status.md)
normalise les statuts COSEPAC (Canada) et LEMV (Québec), en conservant
le statut le plus sévère par espèce, avec un mode `translate = TRUE`
optionnel pour les libellés anglais.
...
récupèrent et analysent les enregistrements GenBank complets ;
[`score_sequence_quality()`](https://taq-community.github.io/QCGenomLandscape/reference/score_sequence_quality.md)
calcule des métriques simples (longueur, %N, %GC) et
[`has_stop_codon_coi()`](https://taq-community.github.io/QCGenomLandscape/reference/has_stop_codon_coi.md)
signale les séquences COI avec un codon stop en phase (code
mitochondrial invertébré), une anomalie typique d’une séquence mal
orientée ou d’un pseudogène.
...
assemble un tableau par espèce (séquences NCBI, marqueurs disponibles,
taxonomie, statuts de conservation).
...
(Keystone). Si les variables `SWIFT_*` ne sont pas configurées, cette
étape est simplement ignorée (avec un avertissement) — le reste du
...
- **Rate limiting** : NCBI (~3 requêtes/s, 10/s avec clé API), BOLD (pas
  de limite stricte documentée mais délai recommandé).
- **Exactitude taxonomique** : dépend de la nomenclature utilisée par
  chaque base ; certaines espèces ont plusieurs noms ou synonymes non
  résolus.
- **Qualité des données** : NCBI accepte des soumissions publiques de
  qualité variable ; BOLD est plus standardisé mais moins diversifié
  taxonomiquement.
- **Biais taxonomiques** : vertébrés et plantes sur-représentés ;
  insectes et araignées dominent chez les arthropodes ; champignons et
  bactéries sous-représentés.
- **Biais géographiques** : les séquences NCBI ne sont pas spécifiques
  au Québec ; le filtrage géographique ne s’applique qu’aux
  enregistrements avec coordonnées.
